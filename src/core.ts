...
export interface Config {
  ...
  username: string; remotePath: string; uploadOnSave: boolean;
...
export const configFields = ["username", "password", "host", "remotePath", "port", "upload_on_save", "discover", "protocol", "passive", "ignore_always", "ignore_upload", "ignore_download", "debug"];
export function parseConfig(value: unknown): Config {
  ...
  for (const k of ['host','username','remotePath']) if (typeof c[k] !== 'string' || !(c[k] as string).trim() || /[\r\n\0]/.test(c[k] as string)) throw new Error(`Invalid field: ${k}.`);
  const remote = c.remotePath as string;
  if (!remote.startsWith('/') || remote.includes('\\') || remote.split('/').includes('..')) throw new Error('remotePath must be absolute and must not contain .. or backslashes.');
  ...
  for (const name of ['upload_on_save']) {
  ...
  return { ...c, remotePath:path.posix.normalize(remote), port, timeout, passive:c.passive ?? true, debug:c.debug ?? false, uploadOnSave:c.upload_on_save ?? false,
...
export function remoteFile(config: Config, relative: string): string { return path.posix.join(config.remotePath, safeRelative(relative)); }
